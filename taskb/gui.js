    ...
    addButton.action = function () {
        ...
        if (username.length < 1) {
            ...
            if (result === "success") {
                ...
                myself.showMakeAnnouncementSuccessPopup();
...
IDE_Morph.prototype.showMakeAnnouncementSuccessPopup = function() {
    var world = this.world();
    var myself = this;
    var popupWidth = 400;
    var popupHeight = 330;
    ...
    if (this.showSuccessPopup) {
        this.showSuccessPopup.destroy();
    }

    this.showSuccessPopup = new DialogBoxMorph();
    this.showSuccessPopup.setExtent(new Point(popupWidth, popupHeight));
    ...
    button.setRight(this.showSuccessPopup.right() - 3);
    button.setTop(this.showSuccessPopup.top() + 2);
    button.action = function () { myself.showSuccessPopup.cancel(); };
    button.drawNew();
    button.fixLayout();
    this.showSuccessPopup.add(button);

    // add title
    this.showSuccessPopup.labelString = "You have just sent an announcement.";
    this.showSuccessPopup.createLabel();
    ...
    successImage.drawNew = function () {
        this.image = newCanvas(this.extent());
        var context = this.image.getContext('2d');
        var picBgColor = myself.showSuccessPopup.color;
        context.fillStyle = picBgColor.toString();
        context.fillRect(0, 0, this.width(), this.height());
        if (this.texture) {
            this.drawTexture(this.texture);
    ...
    successImage.setExtent(new Point(128, 128));
    successImage.setCenter(this.showSuccessPopup.center());
    successImage.setTop(this.showSuccessPopup.top() + 40);
    this.showSuccessPopup.add(successImage);

    // success message
    txt = new TextMorph("You will be notified once everyone has read your message");
    txt.setCenter(this.showSuccessPopup.center());
    txt.setTop(successImage.bottom() + 20);
    this.showSuccessPopup.add(txt);
    txt.drawNew();

    // "got it!" button, closes the dialog.
    okButton = new PushButtonMorph(null, null, "Got it!", null, null, null, "green");
    okButton.setCenter(this.showSuccessPopup.center());
    okButton.setBottom(this.showSuccessPopup.bottom() - 10);
    okButton.action = function() { myself.showSuccessPopup.cancel(); };
    this.showSuccessPopup.add(okButton);

    // popup
    this.showSuccessPopup.drawNew();
    this.showSuccessPopup.fixLayout();
    this.showSuccessPopup.popUp(world);
